...
def test_remind(remind:List[RemindModel2],db:Session=Depends(get_db),a:bool=Depends(get_current_admin)):
    ...
    # change the placement email to depending on the college placement 
...
@stats_router.post("/home/send_reminder/{cid}/{college_id}",include_in_schema=True)
...
@stats_router.get("/download/downloadentireplacedlist")
async def entireplaced(bg_tasks: BackgroundTasks,dba: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    ...
    sql1='''SELECT s.urn, s.full_name, s.email, c.cname, c.package, p.category_placed, s.branch, s.first_name, s.middle_name, s.last_name, s.current_backlogs, s.history_backlogs, s.other_grades, s.ssc_board,
    ...
    s.sem7, s.sem8 ,s.ssc, s.present_addr, s.permanent_addr, s.no_of_x_grades, s.ug_start_year, s.ug_end_year, s.ssc_start_year, s.ssc_end_year, s.hsc_start_year, s.hsc_end_year, s.rank, s.dob
    FROM students s, placed p , company c where s.urn = p.urn and c.cid = p.cid '''   
    
    
    headers=["URN/USN", "Full Name", "Email", "Company", "Package","Category Placed","Branch", "First Name", "Middle Name", "Last Name","Current Backlogs","History Backlogs","Other Grades","SSC Board","HSC Board","Entry To College", 
            ...
            "SSC Start Year", "SSC End Year", "HSC Start Year", "HSC End Year", "Rank","DOB"]
...
async def regcid_list_short(bg_tasks: BackgroundTasks,cid:int,db: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    row=1
    col=0
    
    BE=['CSE','CSBS','ISE','IT','ECE','EI','EEE','MECH','EE','ME','CE','MCA','IP','CV','CTM','PST','BT','ENV','BCA']

    MTECH = [ 'MSC','MCA','Computer_Engineering','Software_Engineering','Data_Science','Biomedical_Signal_Processing','Networking_Internet_Engineering','Industrial_Electronics','Polymer_Science','Energy_Systems_Management','Mechanical_Engineering_PG','Automotive_Electronics','Industrial_Structures','Infrastructure_Engineering_Management','Environmental_PG','Health_Science_Water_Treatment','Material_Science']

    MBA = ['MBA','MBA_Financial_Management','MBA_Retail_Management','MBA_Digital_Management','MBA_Finance','MBA_Marketing','MBA_HR']
    
    c_name = db.query(models.Company.cname).filter(models.Company.cid ==cid).first()
    b=c_name.cname
    with engine.connect() as con:
        sql = """select s.urn,email,degree,branch,full_name,ssc,hsc,ug,pg,current_backlogs,dob,gender,phone,resume_link
        from students s 
inner join registrations a on s.urn = a.urn
where a.cid ={};""".format(cid)
        rs = con.execute(sql)
        output = '{}.xlsx'.format(b)
        ...
        worksheet.write(0,2,"Degree")
        worksheet.write(0,3,"Branch")
        worksheet.write(0,4,"Full Name")
        worksheet.write(0,5,"10th Percentage")
        worksheet.write(0,6,"12th Percentage")
        worksheet.write(0,7,"UG CGPA")
        worksheet.write(0,8,"PG CGPA")
        worksheet.write(0,9,"Current Backlogs")
        worksheet.write(0,10,"DOB")
        worksheet.write(0,11,"Gender")
        worksheet.write(0,12,"Contact Number")
        worksheet.write(0,13,"Resume Link")
        
        for i in rs:
            for j in i.values():
                if j=="male" or j=="M" or j=="Male":
                   j="MALE"
                if j=="female" or j=="F" or j=="Female":
                   j="FEMALE"
                if j == -1:
                    j=""
                else:
        ...
        worksheet.set_column('K:K', 25,format2)
...
async def regcid_list_detailed(bg_tasks: BackgroundTasks,cid:int,reg:Request,db: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    row=1
    col=0
    cname = db.query(models.Company.cname).filter(models.Company.cid ==cid).first()
    with engine.connect() as con:
...
where a.cid ={};""".format(cid)
        ...
        rs = con.execute(sql)
        output = '{}-short.xlsx'.format(cname[0])
        workbook = xlsxwriter.Workbook(output)
...
@stats_router.get("/download/student/total/{college_id}")
async def student_count(college_id:int,reg:Request,db: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    output=db.query(User.branch).filter(User.college_id==college_id).all()
    return len(output)

@stats_router.get("/download/admin/total_company/{college_id}")
async def company_count(college_id:int,reg:Request,db: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    output=db.query(models.Company.cid).filter(models.Company.eligible_college_ids.contains(str(college_id))).all()
    return len(output)

@stats_router.get("/download/admin/companies_so_far/{college_id}")
async def companysofar_count(college_id:int,reg:Request,db: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    # output=db.query(models.Company.cid).filter(models.Company.eligible_college_ids.contains(str(college_id))).filter_by(status=3).all()
    # return len(output)

    """
    Returns the no of unique companies that have 'hired' the students from the college
    """
    print("Companies so far")

    sql = " SELECT COUNT(DISTINCT p.cid) FROM placed p"
    ...
    print(f"No of companies that have hired students from the college: {output}")
    return output

@stats_router.get("/download/admin/offers/{college_id}")
async def offers_count(college_id:int,reg:Request,db: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    output=db.query(models.Placed.pid).filter(models.Placed.college_id==college_id).all()
    return len(output)
    
@stats_router.get("/download/admin/placed_count/{college_id}")
async def placed_count(college_id:int,reg:Request,db: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    output=db.query(models.Placed.urn).filter(models.Placed.college_id==college_id).distinct().all()
    return len(output)
...
async def eligibility(bg_tasks: BackgroundTasks,dba: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    ...
    hrow=0
    hcol=0
    headers=['Company','CSE','CSBS','ISE','IT','ECE','EI','EEE','MECH','IP','CV','CTM','PST','BT','ENV','BCA','MSC_Comp','MSC_Math','MSC_Phy','MSC_Chem','MSC_AnaChem','MSC_Poly','MSC_Bio','MCA',
                  'Computer_Engineering','Software_Engineering','Data_Science','Biomedical_Signal_Processing','Networking_Internet_Engineering',
'Industrial_Electronics','Polymer_Science','Energy_Systems_Management','Mechanical_Engineering_PG','Automotive_Electronics','Industrial_Structures','Infrastructure_Engineering_Management','Environmental_PG','Health_Science_Water_Treatment','Material_Science','MBA_Financial_Management','MBA_Retail_Management','MBA_Digital_Management','MBA_Finance','MBA_Marketing','MBA_HR'
,'Total Placed','Total Registered','SSC Cutoff','HSC Cutoff','UG Cutoff','PG Cutoff','Package']
    
    branches=['CSE','CSBS','ISE','IT','ECE','EI','EEE','MECH','IP','CV','CTM','PST','BT','ENV','BCA','MSC_Comp','MSC_Math','MSC_Phy','MSC_Chem','MSC_AnaChem','MSC_Poly','MSC_Bio','MCA',
                  'Computer_Engineering','Software_Engineering','Data_Science','Biomedical_Signal_Processing','Networking_Internet_Engineering',
'Industrial_Electronics','Polymer_Science','Energy_Systems_Management','Mechanical_Engineering_PG','Automotive_Electronics','Industrial_Structures','Infrastructure_Engineering_Management','Environmental_PG','Health_Science_Water_Treatment','Material_Science','MBA_Financial_Management','MBA_Retail_Management','MBA_Digital_Management','MBA_Finance','MBA_Marketing','MBA_HR']
