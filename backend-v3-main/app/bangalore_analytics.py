...
bangalore_stats_router = APIRouter(
    prefix='/stats/bangalore',
    tags=['stats_bangalore']
    )
...
@bangalore_stats_router.post("/home/test_reminder/{cid}/",include_in_schema=True)
...
@bangalore_stats_router.post("/home/send_reminder/{cid}",include_in_schema=True)
...
@bangalore_stats_router.get("/download/downloadregisteredbutnotplaced", response_description='xlsx')
...
@bangalore_stats_router.get("/download/downloadstudenttable", response_description='xlsx')
async def student_table(bg_tasks: BackgroundTasks,dba: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    ...
    sql1="SELECT branch,s.urn, full_name,email,ssc,hsc,ug,ug_percentage,pg,backlogs,sem1,sem2,sem3,sem4,sem5,sem6,sem7,sem8,current_backlogs,history_backlogs,no_of_x_grades,other_grades,ug_start_year,ug_end_year,ssc_board,ssc_start_year,ssc_end_year,hsc_board,hsc_start_year,hsc_end_year,entry_to_college,rank,gap_in_studies,dob,gender,category,native,parents_name,present_addr,permanent_addr,phone,secondary_phone FROM students s where s.college_id=2"
...
@bangalore_stats_router.get("/download/placed_list/", response_description='xlsx')
async def placed_list(bg_tasks: BackgroundTasks,dba: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    ...
    with engine.connect() as con:
           ...
           rows+=1
           sql2= """SELECT students.urn, students.full_name, students.branch FROM placed INNER JOIN students ON placed.urn = students.urn where students.college_id = 2"""
           rp=con.execute((sql2))
           for row in rp:
               urns.append(str(row['urn']))
               sql3="""SELECT company.cname FROM placed INNER JOIN company ON company.cid = placed.cid WHERE placed.college_id=2 and placed.urn ='{}'""".format(row['urn'])
               rr=con.execute((sql3))
               for j in row.values():
...
@bangalore_stats_router.get("/download/placedBranchWise/", response_description='xlsx')
async def placedbranch(bg_tasks: BackgroundTasks,dba: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    ...
    with engine.connect() as con:
        ...
        sql1 = 'select DISTINCT students.branch from placed INNER JOIN students on placed.urn=students.urn where students.college_id=2'
        ...
        for i in branches:
           ...
           rows+=1
           sql2= """SELECT students.urn, students.full_name, students.branch FROM placed INNER JOIN students ON placed.urn = students.urn WHERE students.college_id=2 and students.branch = '{}'""".format(i)
           rp=con.execute((sql2))
           for row in rp:
               urns.append(str(row['urn']))
               sql3="""SELECT company.cname FROM placed INNER JOIN company ON company.cid = placed.cid WHERE placed.college_id=2 and placed.urn ='{}'""".format(row['urn'])
...
@bangalore_stats_router.get("/download/placedGenderwise/", response_description='xlsx')
async def placedgender(bg_tasks: BackgroundTasks,dba: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    ...
    with engine.connect() as con:    
        ...
        for i in branches:
           worksheet.write(rows,col,i)
           rows+=1
           sql2= """SELECT students.urn, students.full_name, students.branch FROM placed INNER JOIN students ON placed.urn = students.urn WHERE students.college_id=2 and students.gender = '{}'""".format(i)
           rp=con.execute((sql2))
           for row in rp:
               urns.append(str(row['urn']))
               sql3="""SELECT company.cname FROM placed INNER JOIN company ON company.cid = placed.cid WHERE placed.college_id=2 and placed.urn ='{}'""".format(row['urn'])
...
@bangalore_stats_router.get("/download/placedCategoryWise/", response_description='xlsx')
async def category(bg_tasks: BackgroundTasks,dba: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    ...
    with engine.connect() as con:
        ...
        sql1 = 'select DISTINCT students.category from placed INNER JOIN students on placed.urn=students.urn where students.college_id=2'
        ...
        for i in branches:
           worksheet.write(rows,col,i)
           rows+=1
           sql2= """SELECT students.urn, students.full_name, students.branch FROM placed INNER JOIN students ON placed.urn = students.urn WHERE students.college_id=2 and students.category = '{}'""".format(i)
           rp=con.execute((sql2))
           for row in rp:
               urns.append(str(row['urn']))
               sql3="""SELECT company.cname FROM placed INNER JOIN company ON company.cid = placed.cid WHERE placed.college_id=2 and placed.urn ='{}'""".format(row['urn'])
...
@bangalore_stats_router.get("/download/companyregperstudent/", response_description='xlsx')
async def companyregperstudent(bg_tasks: BackgroundTasks,dba: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    ...
    with engine.connect() as con:
        ...
        sql1 = 'SELECT DISTINCT registrations.urn FROM registrations where college_id=2'
        ...
        for i in urns:
            sql2= """SELECT DISTINCT students.urn, students.full_name, students.branch, students.email, students.ssc,students.hsc,students.sem1,students.sem2, students.sem3,students.sem4,students.sem5,students.sem6,students.sem7,students.sem8,students.ug,students.ug_percentage,students.pg,students.backlogs,students.current_backlogs,students.history_backlogs,students.no_of_x_grades,students.other_grades,students.ug_start_year ,students.ug_end_year, students.ssc_board,students.ssc_start_year,students.ssc_end_year,students.hsc_board,students.hsc_start_year,students.hsc_end_year,students.entry_to_college,students.rank,students.gap_in_studies,students.dob,students.gender,students.category,students.native,students.parents_name,students.present_addr,students.permanent_addr,students.phone,students.secondary_phone FROM students WHERE students.college_id=2 and students.urn = '{}'""".format(i)
            sql3="""SELECT company.cname FROM registrations INNER JOIN company ON company.cid = registrations.cid WHERE registrations.college_id=2 and registrations.urn = '{}'""".format(i)
            rp=con.execute(sql2)
            rt =con.execute(sql3)    
...
@bangalore_stats_router.get("/download/downloadentireplacedlist", response_description='xlsx')
async def entireplaced(bg_tasks: BackgroundTasks,dba: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    ...
    FROM students s, placed p , company c where s.urn = p.urn and c.cid = p.cid and s.college_id = 2'''   
...
@bangalore_stats_router.get("/download/downloadnotregisteredyet", response_description='xlsx')
async def notregistered(bg_tasks: BackgroundTasks,dba: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    ...
    sql1="""SELECT * FROM students where urn not in (SELECT DISTINCT urn from registrations r join company c on r.cid=c.cid where c.category!='other' and r.college_id=2 ) and verified=1 and college_id = 2""" 
...
@bangalore_stats_router.get("/download/regcid_list_short/{cid}", response_description='xlsx')
async def regcid_list_short(bg_tasks: BackgroundTasks,cid:int,reg:Request,dba: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    ...
    with engine.connect() as con:
        sql = """select s.urn,email,branch,full_name,ssc,hsc,ug,ug_percentage,backlogs,current_backlogs,pg,dob,gender,phone,resume_link
        from students s 
inner join registrations a on s.urn = a.urn
where s.college_id =2 and a.cid ={};""".format(cid)
        ...
        worksheet.write(0,14,"resume_link")
...
@bangalore_stats_router.get("/download/regcid_list_detailed/{cid}", response_description='xlsx')
...
@bangalore_stats_router.get("/download/regcid_list_detailed_palo/", response_description='xlsx')
async def regcid_list_detailed_palo(bg_tasks: BackgroundTasks,reg:Request,dba: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    ...
    with engine.connect() as con:
        ...
        from students 
where (branch='IT' or branch='ECE' or branch='CSE') and (ug>=7.50) and (current_backlogs='0') and college_id =2;"""
...
# @bangalore_stats_router.post('/analytics/test_csv',status_code=200)
...
# @bangalore_stats_router.get("/download/xlsxtest4", response_description='xlsx')
...
@bangalore_stats_router.get("/download/student/verified")
...
@bangalore_stats_router.get("/download/student/total")
...
@bangalore_stats_router.get("/download/admin/total_company")
...
@bangalore_stats_router.get("/download/admin/companies_so_far")
...
@bangalore_stats_router.get("/download/admin/offers")
...
@bangalore_stats_router.get("/download/admin/placed_count")
...
# @bangalore_stats_router.get("/download/regcid_list1/{cid}",response_description='xlsx')
...
async def studentbranch_list2(bg_tasks: BackgroundTasks,dba: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    ...
    a=[]
    with engine.connect() as con:
        sql1 = 'select DISTINCT students.branch from placed_category INNER JOIN students on placed_category.urn=students.urn and students.college_id=2'
        ...
        for i in branches:
           worksheet.write(rows,col,i)
           rows+=1
           sql2= """SELECT DISTINCT students.urn, students.full_name, students.branch FROM placed_category INNER JOIN students ON placed_category.urn = students.urn WHERE students.college_id=2 and students.branch = '{}'""".format(i)
           rp=con.execute((sql2))
           
           for row in rp:
               urns.append(str(row['urn']))
               sql3="""SELECT company.cname FROM placed_category INNER JOIN company ON company.cid = placed_category.cid WHERE placed_category.college_id=2 and  placed_category.urn ='{}'""".format(row['urn'])
               rr=con.execute((sql3))
...
# @bangalore_stats_router.get("/download/company_reglist/{cid}", response_description='xlsx')
...
#     return FileResponse(path=output,filename=output, background=bg_tasks)
@bangalore_stats_router.get("/download/placedbycompany/", response_description='xlsx')
async def placed_list(bg_tasks: BackgroundTasks,cid:int,dba: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    ...
    with engine.connect() as con:
        ...
        sql = """select DISTINCT students.branch,students.urn,students.full_name,students.gender,students.phone,students.email, students.resume_link from placed INNER JOIN students on placed.urn=students.urn where placed.college_id = 2 and  placed.cid={}""".format(cid)
...
@bangalore_stats_router.get("/download/branch_list2/", response_description='xlsx')
...
async def placed_list(bg_tasks: BackgroundTasks,cid:int,dba: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    ...
    with engine.connect() as con:
        ...
        sql = """select DISTINCT students.branch,students.urn,students.full_name,students.gender,students.phone,students.email from placed INNER JOIN students on placed.urn=students.urn where college_id =2 and placed.cid={}""".format(cid)
...
@bangalore_stats_router.get("/download/eligibilitysheet/", response_description='xlsx')
...
'Corporate_Finance','Digital_Marketing','Retail_Marketing']
    ...
    with engine.connect() as con:
        ...
        for row in rs:
            ...
            for a in branches:
                if a in branch:
                    sql2="""SELECT COUNT(DISTINCT placed.urn) AS "c" FROM placed WHERE placed.urn IN (SELECT urn FROM students WHERE students.branch = '{}') and college_id =2 AND cid ={}""".format(a,comp_id)
                    ra = con.execute(sql2)
                    for j in ra:
                        placed_count=str(j['c'])
                    sql3="""SELECT COUNT(DISTINCT registrations.urn) AS "c" FROM registrations WHERE registrations.urn IN (SELECT urn FROM students WHERE students.branch = '{}') and college_id =2 AND cid ={}""".format(a,comp_id)
                    rb=con.execute(sql3)
                    for k in rb:
                         reg_count=str(k['c'])
                    ...
                    col+=1
            sql4="""SELECT COUNT(DISTINCT placed.urn) AS "c" FROM placed WHERE  placed.college_id =2 and placed.cid ={}""".format(comp_id)
            sql5="""SELECT COUNT(DISTINCT registrations.urn) AS "c" FROM registrations WHERE registrations.college_id =2 and registrations.cid ={}""".format(comp_id)
            rc = con.execute(sql4)
            for j in rc:
                    totplaced=str(j['c'])
            rd = con.execute(sql5)
...
@bangalore_stats_router.get("/download/offerperstudent", response_description='xlsx')
async def offer(bg_tasks: BackgroundTasks,dba: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    ...
    for i in headers:
        ...
        hcol+=1
    with engine.connect() as con:
        sql="""SELECT DISTINCT placed.urn FROM placed where placed.college_id =2 """
        rs = con.execute(sql)
        for row in rs:
            urns.append(str(row['urn']))
        ...
        for i in urns:
            sql1 = """select s.urn,full_name,branch,email,ssc,hsc,sem1,sem2,sem3,sem4,sem5,sem6,sem7,sem8,ug,ug_percentage,pg,backlogs,current_backlogs,history_backlogs,no_of_x_grades,other_grades,ug_start_year,ug_end_year,ssc_board,ssc_start_year,ssc_end_year,hsc_board,hsc_start_year,hsc_end_year,entry_to_college,rank,gap_in_studies,dob,gender,category,native,parents_name,present_addr,permanent_addr,phone,secondary_phone
                from students s where s.college_id = 2 and s.urn='{}'""".format(i)
            rp=con.execute(sql1)
            for x in rp:
                for y in x.values():
                    worksheet.write(rows, col, y)
            ...
            sql2="""SELECT count(*) FROM placed WHERE placed.college_id =2 and placed.urn = '{}'""".format(i)
            rs=con.execute(sql2)
            for x in rs:
                compcount=str(x[0])
            ...
            sql3="""SELECT company.cname FROM placed INNER JOIN company ON company.cid = placed.cid WHERE placed.college_id =2 and placed.urn ='{}'""".format(i)
            rr=con.execute(sql3)
            for k in rr:
                    worksheet.write(rows, col, k['cname'])
                    col+=1
...
@bangalore_stats_router.get("/download/tier1detailed", response_description='xlsx')
...
where s.college_id = 2 and a.category_placed='tier1';"""
        ...
        for i in rs:
            ...
            sql3="""SELECT company.cname FROM placed INNER JOIN company ON company.cid = placed.cid WHERE placed.college_id =2 and placed.urn ='{}' AND placed.category_placed='tier1'""".format(i['urn'])   
            rr=con.execute(sql3)
            for k in rr:
...
@bangalore_stats_router.get("/download/tier1short", response_description='xlsx')
...
@bangalore_stats_router.get("/download/tier2detailed", response_description='xlsx')
...
@bangalore_stats_router.get("/download/tier2short", response_description='xlsx')
...
@bangalore_stats_router.get("/download/dreamdetailed", response_description='xlsx')
...
@bangalore_stats_router.get("/download/dreamshort", response_description='xlsx')
...
@bangalore_stats_router.get("/download/coredetailed", response_description='xlsx')
...
@bangalore_stats_router.get("/download/coreshort", response_description='xlsx')
...
@bangalore_stats_router.get("/download/specialdetailed", response_description='xlsx')
...
@bangalore_stats_router.get("/download/specialshort", response_description='xlsx')
...
@bangalore_stats_router.get("/download/interndetailed", response_description='xlsx')
...
@bangalore_stats_router.get("/download/internshort", response_description='xlsx')
async def specialshort(bg_tasks: BackgroundTasks,dba: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    row=1
    col=0
...
@bangalore_stats_router.get("/graph1")
def graph1(a:bool=Depends(get_current_admin)):
    with engine.connect() as con:
        ...
        ans.append(count)
        return ans
@bangalore_stats_router.get("/placed_branchwise_graph")
def placed_branchwise_graph(a:bool=Depends(get_current_admin)):
    with engine.connect() as con:
        sql="""select count(*) as count,branch from students s inner join placed a on s.urn = a.urn where s.college_id = 2 group by branch;"""    
        sql1 = """select count(*) as count,branch from students where college_id = 2 group by branch;"""
        ...
        ans.append(count)
        return ans
@bangalore_stats_router.get("/placed_branchwise_stats")
def placed_branchwise_stats(a:bool=Depends(get_current_admin)):
    with engine.connect() as con:
        ...
        return branchwise
@bangalore_stats_router.get("/placed_branchwise_placed_category_wise_graph")
def placed_branchwise_placed_tier2_graph(a:bool=Depends(get_current_admin)):
    with engine.connect() as con:
        sql="""select count(*) as count,branch from students s inner join placed a on s.urn = a.urn where s.college_id = 2 and  (a.category_placed = 'tier2') group by branch;"""  
        rs = con.execute(sql)
        branch=[]
        count=[]
        for i in rs:
        ...
        ans.append(count)
        return ans
@bangalore_stats_router.get("/placed_branchwise_placedratio_graph")
def placed_branchwise_placedratio_graph(a:bool=Depends(get_current_admin)):
    with engine.connect() as con:
        sql="""select count(*) as count,branch from students s inner join placed a on s.urn = a.urn where s.college_id = 2 group by branch;"""
        branchwise_count="""select count(*) as count,branch from students where college_id = 2 group by branch;"""
        rs = con.execute(sql)
        ...
        return placed_ratio
@bangalore_stats_router.get("/genderwise_placed_ratio")
def genderwise_placed_pie_chart(a:bool=Depends(get_current_admin)):
    with engine.connect() as con:
        sql="""select s.gender from students s inner join placed a on s.urn = a.urn where s.college_id = 2;"""
        rs = con.execute(sql)
        gender={"male":[],"female":[]}
        count_male=0
        count_female=0
...
@bangalore_stats_router.get("/tierwise_placed_count")
def tierwisewise_placed_bar_chart(a:bool=Depends(get_current_admin)):
    with engine.connect() as con:
        sql="""select count(*) as count,category_placed from placed where college_id = 2 group by category_placed;"""
        rs = con.execute(sql)
        tier=[]
        count=[]
        for i in rs:
...
@bangalore_stats_router.get("/average_package")
def package_analysis(a:bool=Depends(get_current_admin)):
    with engine.connect() as con:
        sql="""select c.package from company c inner join placed a on c.cid = a.cid where a.college_id = 2 ;"""
        rs = con.execute(sql)
        package=[]
        dummy_package=[]
        for i in rs:
        ...
        return ans
@bangalore_stats_router.get("/studentside_success_rate_per_student/{urn}")
...
@bangalore_stats_router.get("/success_rate_per_student", response_description='xlsx')
async def student_registrations_vs_placement_count_ratio(bg_tasks: BackgroundTasks,dba: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    ...
    with engine.connect() as con:
        ...
        sql0="""select urn from students where college_id = 2;"""
        rs = con.execute(sql0)
...
@bangalore_stats_router.get("/download/branchwisecompany/", response_description='xlsx')
async def placedbranch_company(bg_tasks: BackgroundTasks,dba: Session = Depends(get_db),a:bool=Depends(get_current_admin)):
    ...
    with engine.connect() as con:
        for i in branches:
            ...
            sql2= """select company.cname from company  where company.eligible_college_ids like '%2%'and  company.branch like '%%{}%%'""".format(i)
            rp=con.execute((sql2))
...
@bangalore_stats_router.get("/branchwise_average_package")
def branchwise_package_analysis(a:bool=Depends(get_current_admin)):
    with engine.connect() as con:
        sql="""select c.package,s.branch from company c inner join placed a on c.cid = a.cid inner join students s on a.urn = s.urn where s.college_id = 2 ;"""
        rs = con.execute(sql)
...
# @bangalore_stats_router.get("/download/xlsxtest/{cid}", response_description='xlsx')
...
# @bangalore_stats_router.get("/download/placed_list/{cid}", response_description='xlsx')
...
# @bangalore_stats_router.get("/download/regcid_count/{cid}", response_description='xlsx')
...
# @bangalore_stats_router.get("/download/xlsxtest8", response_description='xlsx')
